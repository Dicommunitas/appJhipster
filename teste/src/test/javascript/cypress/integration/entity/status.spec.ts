...
describe('Status e2e test', () => {
  ...
  const statusSample = { descricao: 'Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci50eHQ=', prazo: '2022-04-26' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/problemas',
      body: {"dataVerificacao":"2022-04-25","descricao":"Account transmitter","criticidade":"IMEDIATA","impacto":"Madeira Brand Architect","dataFinalizacao":"2022-04-26","foto":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","fotoContentType":"unknown"},
    }).then(({ body }) => {
      problema = body;
    });
  });
  ...
  describe('new Status page', () => {
    ...
    it.skip('should create an instance of Status', () => {
      ...
      cy.get(`[data-cy="prazo"]`).type('2022-04-26').should('have.value', '2022-04-26');

      cy.get(`[data-cy="dataResolucao"]`).type('2022-04-25').should('have.value', '2022-04-25');

      cy.get(`[data-cy="createdDate"]`).type('2022-04-26T02:10').should('have.value', '2022-04-26T02:10');

      cy.get(`[data-cy="lastModifiedBy"]`).type('Solutions').should('have.value', 'Solutions');

      cy.get(`[data-cy="lastModifiedDate"]`).type('2022-04-25T23:27').should('have.value', '2022-04-25T23:27');
