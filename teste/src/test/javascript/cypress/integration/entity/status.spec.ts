...
describe('Status e2e test', () => {
  ...
  const statusSample = { descricao: 'Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci50eHQ=', prazo: '2022-04-12' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/problemas',
      body: {"dataVerificacao":"2022-04-12","descricao":"Account transmitter","criticidade":"IMEDIATA","impacto":"Madeira Brand Architect","dataFinalizacao":"2022-04-12","foto":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","fotoContentType":"unknown"},
    }).then(({ body }) => {
      problema = body;
    });
  });
  ...
  describe('new Status page', () => {
    ...
    it.skip('should create an instance of Status', () => {
      ...
      cy.get(`[data-cy="prazo"]`).type('2022-04-12').should('have.value', '2022-04-12');

      cy.get(`[data-cy="dataResolucao"]`).type('2022-04-12').should('have.value', '2022-04-12');

      cy.get(`[data-cy="createdDate"]`).type('2022-04-12T15:04').should('have.value', '2022-04-12T15:04');

      cy.get(`[data-cy="lastModifiedBy"]`).type('Solutions').should('have.value', 'Solutions');

      cy.get(`[data-cy="lastModifiedDate"]`).type('2022-04-12T12:21').should('have.value', '2022-04-12T12:21');
